class StoreImagesController < ApplicationController
  ...
  # GET /store_images
  # GET /store_images.json
  def index
    @store_images = StoreImage.all
  end

  # GET /store_images/1
  # GET /store_images/1.json
  def show
  end
  ...
  def update
    respond_to do |format|
      if @store_image.update(store_image_params)
        format.html { redirect_to @store_image, notice: 'Store image was successfully updated.' }
  ...
  private
    ...
    def store_image_params
      params.require(:store_image).permit(:store_id, :image_id, :position)
    end
end
